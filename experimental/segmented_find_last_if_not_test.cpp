...
// Runs segmented_find_last_if_not over a sub-range whose segmentation shape is
...
template<class Pred>
struct find_last_if_not_shape_check
{
   Pred pred;

   explicit find_last_if_not_shape_check(Pred p) : pred(p) {}
   ...
   void operator()(Cont& c, std::size_t n, const char* spec) const
   {
      ...
      const iter_t r = segmented_find_last_if_not(first, last, pred);
...
template<class Pred>
find_last_if_not_shape_check<Pred> make_find_last_if_not_check(Pred p)
{  return find_last_if_not_shape_check<Pred>(p);   }

template<class Pred>
void run_find_last_if_not_shapes(const int* vals, std::size_t n, Pred pred)
{
   test_detail::for_each_shape_all<int>(vals, n, -999, make_find_last_if_not_check(pred));
   //Forward iterators reach a separate segmented implementation.
   test_detail::for_each_shape_all_fwd<int>(vals, n, -999, make_find_last_if_not_check(pred));
}

void test_find_last_if_not_shape_matrix()
{
   ...
   for(std::size_t s = 0; s != sizeof(sizes)/sizeof(sizes[0]); ++s) {
      ...
      {  //Every value appears twice, so the last match and the first match
         ...
         for(std::size_t v = 0; v <= n + 1u; ++v) {
            ...
            run_find_last_if_not_shapes(vals, n, not_equals_val(target));
         }
      }
      {  //Exactly one element fails the predicate, at each position in turn:
         //whenever it is not in the final segment the result has to survive
         //the segments walked after it.
         for(std::size_t p = 0; p != n; ++p) {
            int vals[16];
            for(std::size_t i = 0; i != 16u; ++i)
               vals[i] = (i == p) ? -int(i + 1u) : int(i + 1u);
            run_find_last_if_not_shapes(vals, n, is_positive());
         }
      }
      {  //Every element satisfies the predicate, and the out-of-range filler
         //does not, so an overrun would report a match.
         int vals[16];
         for(int i = 0; i != 16; ++i)
            vals[i] = i + 1;
         run_find_last_if_not_shapes(vals, n, is_positive());
...
void test_find_last_if_not_single_segment_sentinel()
{
   test_detail::seg_vector<int> sv;
   int a[] = {-9, 1, -2, 3, -4, 5, -9};
   ...
   BOOST_TEST(segmented_find_last_if_not(first, test_detail::make_sentinel(last), is_positive())
              == test_detail::iter_at(sv, 4));
   BOOST_TEST(segmented_find_last_if_not(first, test_detail::make_sentinel(last), not_equals_val(-9)) == last);
}
...
void test_find_last_if_not_forward_match_in_earlier_segment()
{
   ...
   BOOST_TEST(segmented_find_last_if_not(sv.begin(), sv.end(), is_positive())
   ...
   BOOST_TEST(segmented_find_last_if_not(sv.begin(), last, is_positive()) == test_detail::iter_at(sv, 1));
   BOOST_TEST(segmented_find_last_if_not(sv.begin(), last, not_equals_val(6)) == last);
}

void test_find_last_if_not_forward_match_in_earlier_segment_seg2()
{
   ...
   BOOST_TEST(segmented_find_last_if_not(sv2.begin(), sv2.end(), is_positive())
              == test_detail::iter_at(sv2, 1));

   const iter_t last = test_detail::iter_at(sv2, 5);
   BOOST_TEST(segmented_find_last_if_not(sv2.begin(), last, is_positive()) == test_detail::iter_at(sv2, 1));
