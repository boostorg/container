...
// Satisfied by several elements at once, so that the early exit has to stop at
...
// Runs segmented_find_if over a range whose segmentation shape is dictated by
...
template<class Pred>
struct find_if_shape_check
{
   Pred pred;

   explicit find_if_shape_check(Pred p) : pred(p) {}
   ...
   void operator()(Cont& c, std::size_t n, const char* spec) const
   {
      ...
      const iter_t r = segmented_find_if(first, last, pred);
...
template<class Pred>
void run_find_if_shapes(const int* vals, std::size_t n, Pred pred)
{
   test_detail::for_each_shape_all<int>(vals, n, -999, find_if_shape_check<Pred>(pred));
   test_detail::for_each_shape_all_fwd<int>(vals, n, -999, find_if_shape_check<Pred>(pred));
}

void test_find_if_shape_matrix()
{
   int vals[16];
   for(int i = 0; i != 16; ++i)
      vals[i] = i + 1;
   ...
   for(std::size_t s = 0; s != sizeof(sizes)/sizeof(sizes[0]); ++s) {
      ...
      // The satisfied element at the first position, at every interior one and
      ...
      for(std::size_t v = 0; v <= n + 1u; ++v)
         run_find_if_shapes(vals, n, equals_val((v <= n) ? int(v) : -999));
      // Predicates satisfied by many elements: all of them, every second one
      // and every third one, so a match in an earlier segment coexists with
      // matches in later ones.
      run_find_if_shapes(vals, n, multiple_of(1));
      run_find_if_shapes(vals, n, multiple_of(2));
      run_find_if_shapes(vals, n, multiple_of(3));
      // Satisfied by nothing at all, guard included.
      run_find_if_shapes(vals, n, is_negative());
   }
}
...
// would match just before its start, and a negative guard just past its end.
//----------------------------------------------------------------------------

void test_find_if_single_segment_interior()
{
   test_detail::seg_vector<int> sv;
   ...
   BOOST_TEST(segmented_find_if(first, last, equals_val(20)) == first);
   BOOST_TEST(segmented_find_if(first, last, equals_val(50)) == test_detail::iter_at(sv, 4));
   BOOST_TEST(segmented_find_if(first, last, equals_val(35)) == last);
   BOOST_TEST(segmented_find_if(first, last, equals_val(10)) == last);
   BOOST_TEST(segmented_find_if(first, last, is_negative()) == last);
}

void test_find_if_single_segment_sentinel()
{
   test_detail::seg_vector<int> sv;
   int a[] = {10, 20, 30, 40, 50, -60};
   sv.add_segment_range(a, a + 6);
   ...
   BOOST_TEST(segmented_find_if(first, test_detail::make_sentinel(last), equals_val(20)) == first);
   BOOST_TEST(segmented_find_if(first, test_detail::make_sentinel(last), equals_val(50)) == test_detail::iter_at(sv, 4));
   BOOST_TEST(segmented_find_if(first, test_detail::make_sentinel(last), is_negative()) == last);
}

void test_find_if_single_segment_seg2_inner_multi()
{
   ...
   BOOST_TEST(segmented_find_if(first, last, equals_val(20)) == first);
   BOOST_TEST(segmented_find_if(first, last, equals_val(40)) == test_detail::iter_at(sv2, 3));
   BOOST_TEST(segmented_find_if(first, last, equals_val(60)) == test_detail::iter_at(sv2, 5));
   BOOST_TEST(segmented_find_if(first, last, equals_val(10)) == last);
   BOOST_TEST(segmented_find_if(first, last, is_negative()) == last);
}

void test_find_if_single_segment_seg2_single_inner()
{
   test_detail::seg_vector<int> inner;
   ...
   BOOST_TEST(segmented_find_if(first, last, equals_val(20)) == first);
   BOOST_TEST(segmented_find_if(first, last, equals_val(50)) == test_detail::iter_at(sv2, 4));
   BOOST_TEST(segmented_find_if(first, last, equals_val(35)) == last);
   BOOST_TEST(segmented_find_if(first, last, equals_val(10)) == last);
   BOOST_TEST(segmented_find_if(first, last, equals_val(60)) == last);
}
...
void test_find_if_single_segment_forward()
{
   ...
   BOOST_TEST(segmented_find_if(sv.begin(), whole_last, equals_val(10)) == sv.begin());
   BOOST_TEST(segmented_find_if(sv.begin(), whole_last, equals_val(60)) == test_detail::iter_at(sv, 5));
   BOOST_TEST(segmented_find_if(sv.begin(), whole_last, is_negative()) == whole_last);
...
void test_find_if_forward_match_before_last_segment()
{
   typedef test_detail::seg_vector<int, std::forward_iterator_tag> cont_t;
   typedef cont_t::iterator iter_t;
   ...
   BOOST_TEST(segmented_find_if(sv.begin(), last, equals_val(20)) == test_detail::iter_at(sv, 1));
   BOOST_TEST(segmented_find_if(sv.begin(), last, equals_val(40)) == test_detail::iter_at(sv, 3));
...
int main()
{
   ...
   test_find_if_single_segment_interior();
   test_find_if_single_segment_sentinel();
   test_find_if_single_segment_seg2_inner_multi();
