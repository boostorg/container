...
// Runs segmented_any_of over a range whose segmentation shape is dictated by a
...
// segmented_any_of is the negation of segmented_none_of, so what this checks
// on top of the none_of matrix is that the negation is the only difference.
template<class Pred>
struct any_of_shape_check
{
   Pred pred;

   explicit any_of_shape_check(Pred p) : pred(p) {}
   ...
   void operator()(Cont& c, std::size_t n, const char* spec) const
   {
      ...
      for(std::size_t i = 0; i != flat.size(); ++i) {
         if(pred(flat[i])) { expected = true; break; }
      }

      BOOST_TEST_EQ(segmented_any_of(first, last, pred), expected);
      BOOST_TEST(spec != 0);
   }
};
...
template<class Pred>
void run_any_of_shapes(const int* vals, std::size_t n, Pred pred)
{
   test_detail::for_each_shape_all<int>(vals, n, -999, any_of_shape_check<Pred>(pred));
   test_detail::for_each_shape_all_fwd<int>(vals, n, -999, any_of_shape_check<Pred>(pred));
}

void test_any_of_shape_matrix()
{
   int vals[16];
   for(int i = 0; i != 16; ++i)
      vals[i] = i + 1;
   ...
   for(std::size_t s = 0; s != sizeof(sizes)/sizeof(sizes[0]); ++s) {
      ...
      // lies past the end and must never be reported as a hit.
      for(std::size_t v = 0; v <= n + 1u; ++v)
         run_any_of_shapes(vals, n, equals_val((v <= n) ? int(v) : -999));
      ...
      // later ones.
      run_any_of_shapes(vals, n, multiple_of(1));
      run_any_of_shapes(vals, n, multiple_of(2));
      run_any_of_shapes(vals, n, multiple_of(3));
      // Hit by nothing at all, guard included.
      run_any_of_shapes(vals, n, is_negative());
   }
}
...
void test_any_of_single_segment_interior()
{
   test_detail::seg_vector<int> sv;
   ...
   BOOST_TEST(segmented_any_of(first, last, equals_val(20)));
   BOOST_TEST(segmented_any_of(first, last, equals_val(50)));
   BOOST_TEST(!segmented_any_of(first, last, equals_val(35)));
   BOOST_TEST(!segmented_any_of(first, last, equals_val(10)));
   BOOST_TEST(!segmented_any_of(first, last, is_negative()));
}

void test_any_of_single_segment_sentinel()
{
   test_detail::seg_vector<int> sv;
   int a[] = {10, 20, 30, 40, 50, -60};
   sv.add_segment_range(a, a + 6);
   ...
   BOOST_TEST(segmented_any_of(first, test_detail::make_sentinel(last), equals_val(20)));
   BOOST_TEST(segmented_any_of(first, test_detail::make_sentinel(last), equals_val(50)));
   BOOST_TEST(!segmented_any_of(first, test_detail::make_sentinel(last), is_negative()));
}

void test_any_of_single_segment_seg2_inner_multi()
{
   ...
   BOOST_TEST(segmented_any_of(first, last, equals_val(20)));
   BOOST_TEST(segmented_any_of(first, last, equals_val(40)));
   BOOST_TEST(segmented_any_of(first, last, equals_val(60)));
   BOOST_TEST(!segmented_any_of(first, last, equals_val(10)));
   BOOST_TEST(!segmented_any_of(first, last, is_negative()));
}

void test_any_of_single_segment_seg2_single_inner()
{
   test_detail::seg_vector<int> inner;
...
void test_any_of_single_segment_forward()
{
   ...
   BOOST_TEST(segmented_any_of(sv.begin(), whole_last, equals_val(10)));
   BOOST_TEST(segmented_any_of(sv.begin(), whole_last, equals_val(60)));
   BOOST_TEST(!segmented_any_of(sv.begin(), whole_last, is_negative()));
...
void test_any_of_forward_hit_before_last_segment()
{
   typedef test_detail::seg_vector<int, std::forward_iterator_tag> cont_t;
   typedef cont_t::iterator iter_t;
   ...
   BOOST_TEST(segmented_any_of(sv.begin(), last, equals_val(20)));
   BOOST_TEST(segmented_any_of(sv.begin(), last, equals_val(40)));
   BOOST_TEST(!segmented_any_of(sv.begin(), last, equals_val(80)));
}

//////////////////////////////////////////////////////////////////////////////
// Predicate application count.
//
// [alg.any.of] mandates "At most last - first applications of the predicate".
// The lower bound is not in the standard but follows from the answer: true
// cannot be returned without having applied pred to every element up to and
// including the first hit, and false cannot be returned without having applied
// it to all of them.  Bracketing the count catches an element retested when
// the scan crosses a segment boundary.
//////////////////////////////////////////////////////////////////////////////

template<class Pred>
struct any_of_count_check
{
   Pred pred;

   explicit any_of_count_check(Pred p) : pred(p) {}
   ...
   void operator()(Cont& c, std::size_t n, const char* spec) const
   {
      ...
      segmented_any_of(c.begin(), test_detail::iter_at(c, n),
...
template<class Pred>
void run_any_of_count_shapes(const int* vals, std::size_t n, Pred pred)
{
   test_detail::for_each_shape_all<int>(vals, n, -999, any_of_count_check<Pred>(pred));
   test_detail::for_each_shape_all_fwd<int>(vals, n, -999, any_of_count_check<Pred>(pred));
}

void test_any_of_predicate_count()
{
   int vals[16];
   for(int i = 0; i != 16; ++i)
      vals[i] = i + 1;
   ...
   for(std::size_t s = 0; s != sizeof(sizes)/sizeof(sizes[0]); ++s) {
      const std::size_t n = sizes[s];
      for(std::size_t v = 0; v <= n + 1u; ++v)
         run_any_of_count_shapes(vals, n, equals_val((v <= n) ? int(v) : -999));
      run_any_of_count_shapes(vals, n, multiple_of(1));
      run_any_of_count_shapes(vals, n, multiple_of(3));
      run_any_of_count_shapes(vals, n, is_negative());
   }
}

int main()
{
   test_any_of_shape_matrix();
   test_any_of_empty();
   test_any_of_non_segmented();
   test_any_of_sentinel_segmented();
   test_any_of_sentinel_non_segmented();
   test_any_of_single_segment_interior();
   test_any_of_single_segment_sentinel();
   test_any_of_single_segment_seg2_inner_multi();
   test_any_of_single_segment_seg2_single_inner();
   test_any_of_single_segment_forward();
   test_any_of_forward_hit_before_last_segment();
   test_any_of_predicate_count();
