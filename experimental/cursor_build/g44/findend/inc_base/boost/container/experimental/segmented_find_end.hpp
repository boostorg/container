...
segduo<SegIt, FwdIt2> find_end_verify
   (SegIt it, SegIt last, FwdIt2 s_it, Sent2 s_last, BinaryPred pred, segmented_iterator_tag)
{
   ...
   typedef segduo<SegIt, FwdIt2>          return_t;
   ...
   for(;;) {
      ...
      const local_iterator le = last_seg ? traits::local(last) : traits::end(sfirst);
      {
         ...
         s_it = r.second;
         if(BOOST_UNLIKELY(r.first != le || s_it == s_last))
      ...
      for(++sfirst; sfirst != slast; ++sfirst) {
         ...
         s_it = r.second;
         if(BOOST_UNLIKELY(r.first != me || s_it == s_last))
            return return_t(traits::compose(sfirst, r.first), s_it);
      }
    ...
    const std::forward_iterator_tag&)
{
   ...
   for(;;) {
      ...
      if(s_next == s_last) {
         ...
         FwdIt1 it = cur;
         ++it;
         const segduo<FwdIt1, FwdIt2> r = (find_end_verify)
