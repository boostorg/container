...
{
   while (first != last) {
      //Both scan loops are manually unrolled x2 so that one loop iteration
      //covers two elements: the ~50-byte loop body then costs one 64-byte
      //fetch window per element no matter where the loop lands in code,
      //instead of two windows per element when a ~25-byte body happens to
   ...
   (It f, It const f_end, It const l_beg, It l, Pred pred, non_segmented_iterator_tag, const Cat &)
{
   while (f != f_end) {
      while (pred(*f)) {
         if (BOOST_UNLIKELY(++f == f_end))
            goto duo_ret;
      }

      do {
         if (BOOST_UNLIKELY(l == l_beg))
            goto duo_ret;
         --l;
      } while (!pred(*l));
