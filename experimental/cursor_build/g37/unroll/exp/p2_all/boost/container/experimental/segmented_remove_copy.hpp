   ...
   (SrcIter first, Sent last, DstIter dst_first, DstSent dst_last, const T& value, DstTag, SrcCat)
{
   ...
   //line.  Each step is the single-element body verbatim, so value is still
   //compared once per element and never past last.
   #define BOOST_CONTAINER_REMOVE_COPY_STEP()           \
      if(!(*first == value)) {                          \
   ...
   while(first != last) {
      BOOST_CONTAINER_REMOVE_COPY_STEP()
      if(first == last) break;
      BOOST_CONTAINER_REMOVE_COPY_STEP()
      if(first == last) break;
      BOOST_CONTAINER_REMOVE_COPY_STEP()
   ...
   #undef BOOST_CONTAINER_REMOVE_COPY_STEP
   out_path:
   return segduo<SrcIter, DstIter>(first, dst_first);
}
