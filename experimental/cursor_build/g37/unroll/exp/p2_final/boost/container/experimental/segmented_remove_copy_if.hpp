   ...
   (SrcIter first, Sent last, DstIter dst_first, DstSent dst_last, Pred pred, DstTag, SrcCat)
{
   ...
   //The unroll is written out instead of asked for with a pragma: pragmas are
   //disabled by default, and with a single-element body Clang keeps the store
   //inline and branches over it, spending a taken branch on every element that
   //is skipped.  A wider straight-line body makes it sink the store out of
   //line.  Each step is the single-element body verbatim, so pred is still
   //applied once per element and never past last.
   #define BOOST_CONTAINER_REMOVE_COPY_IF_STEP()        \
      if(!pred(*first)) {                               \
   ...
   while(first != last) {
      BOOST_CONTAINER_REMOVE_COPY_IF_STEP()
      if(first == last) break;
      BOOST_CONTAINER_REMOVE_COPY_IF_STEP()
      if(first == last) break;
      BOOST_CONTAINER_REMOVE_COPY_IF_STEP()
   ...
   #undef BOOST_CONTAINER_REMOVE_COPY_IF_STEP
   out_path:
   return segduo<SrcIter, DstIter>(first, dst_first);
}
