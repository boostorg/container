...
// so the treatment of equal elements is observable in every case.
// Their union is {1, 2, 3, 4, 5, 8, 9}.
//////////////////////////////////////////////////////////////////////////////

// M4: single-segment first input, flat second input.
void test_set_union_single_segment_input1()
{
...
void test_set_union_single_segment_both_inputs()
{
   test_detail::seg_vector<int> sv1;
   int a[] = {-100, 1, 2, 3, 5, 8, 100};
   sv1.add_segment_range(a, a + 7);
...
void test_set_union_single_segment_output()
{
   test_detail::seg_vector<int> sv1;
   int a1[] = {1, 2};
   int a2[] = {3};
   ...
   iter_t r = segmented_set_union(sv1.begin(), sv1.end(), sv2.begin(), sv2.end(), out.begin());
...
void test_set_union_single_segment_inputs_and_output()
{
   test_detail::seg_vector<int> sv1;
   int a[] = {-100, 1, 2, 3, 5, 8, 100};
   sv1.add_segment_range(a, a + 7);
...
// M2: single-segment inputs, multi-segment output; the written range crosses
// two output segments and the residue drain lands in a third.
void test_set_union_single_segment_inputs_multi_output()
{
   test_detail::seg_vector<int> sv1;
   int a[] = {-100, 1, 2, 3, 5, 8, 100};
   sv1.add_segment_range(a, a + 7);
...
void test_set_union_single_segment_first_input_exhausted()
{
   test_detail::seg_vector<int> sv1;
   int a[] = {-100, 1, 2, 100};
   ...
   BOOST_TEST(r == test_detail::iter_at(out, 6));
   boost::container::vector<int> got = test_detail::flatten_all_ints(out);
   BOOST_TEST_EQ(got.size(), 10u);
   int expected[] = {1, 2, 3, 4, 5, 6, -1, -1, -1, -1};
   for(std::size_t i = 0; i < got.size(); ++i)
      BOOST_TEST_EQ(got[i], expected[i]);
}

// Mirror image: the second input is exhausted first.
void test_set_union_single_segment_second_input_exhausted()
{
   test_detail::seg_vector<int> sv1;
...
void test_set_union_single_segment_seg2_inputs()
{
   int a[] = {1, 2, 3, 5, 8};
   int b[] = {2, 3, 4, 8, 9};
...
void test_set_union_single_segment_empty_input()
{
   test_detail::seg_vector<int> sv1;
   int a[] = {1, 2, 3, 5, 8, 100};
   ...
   boost::container::vector<int>::iterator r =
      segmented_set_union(mid, mid, b, b + 3, out.begin());
...
void test_set_union_single_segment_sentinel()
{
   test_detail::seg_vector<int> sv1;
   int a[] = {-100, 1, 2, 3, 5, 8, 100};
   sv1.add_segment_range(a, a + 7);
...
void test_set_union_single_segment_with_comp()
{
   test_detail::seg_vector<int> sv1;
   ...
   boost::container::vector<int> got = test_detail::flatten_all_ints(out);
   BOOST_TEST_EQ(got.size(), 12u);
   int expected[] = {9, 8, 5, 4, 3, 2, 1, -1, -1, -1, -1, -1};
   for(std::size_t i = 0; i < got.size(); ++i)
      BOOST_TEST_EQ(got[i], expected[i]);
}
...
struct set_union_shape_check
{
   ...
   explicit set_union_shape_check(std::size_t e) : extra(e) {}
                   ...
                   C2& c2, std::size_t n2, const char* s2) const
   {
      ...
      // Naive reference: every element of either range, duplicates taken the
      // larger number of times the two ranges hold them.
...
void test_set_union_shape_matrix()
{
   ...
   for(std::size_t p = 0; p != sizeof(pairs)/sizeof(pairs[0]); ++p) {
      for(std::size_t e = 0; e != sizeof(extras)/sizeof(extras[0]); ++e) {
         test_detail::for_each_shape2_all<int, int>
            (v1, pairs[p][0], v2, pairs[p][1], -999, set_union_shape_check(extras[e]));
      }
   }
}
...
int main()
{
   ...
   // Single-segment coverage:
   test_set_union_single_segment_input1();
   test_set_union_single_segment_input2();
   test_set_union_single_segment_both_inputs();
   test_set_union_single_segment_output();
   test_set_union_single_segment_inputs_and_output();
   test_set_union_single_segment_inputs_multi_output();
   test_set_union_single_segment_first_input_exhausted();
   test_set_union_single_segment_second_input_exhausted();
   test_set_union_single_segment_seg2_inputs();
   test_set_union_single_segment_empty_input();
