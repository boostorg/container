...
segduo<InIter2, OutIter> set_difference_scan
...
segduo<InIter2, OutIter> set_difference_scan(SegIt first, SegIt last, InIter2 first2, Sent2 last2, OutIter result, Comp comp,
   segmented_iterator_tag)
{
   ...
   if(scur == slast) {
      ...
      segduo<InIter2, OutIter> r = set_difference_scan(lcur, traits::end(scur), first2, last2, result, comp, is_local_seg_t());

      for(++scur; scur != slast; ++scur)
         r = set_difference_scan(traits::begin(scur), traits::end(scur), r.first, last2, r.second, comp, is_local_seg_t());

      return set_difference_scan(traits::begin(scur), traits::local(last), r.first, last2, r.second, comp, is_local_seg_t());
   }
}
...
{
   return set_difference_scan(first1, last1, first2, last2, result, comp, segmented_iterator_tag()).second;
}
...
segmented_set_difference_dispatch
   (InIter1 first1, Sent1 last1, InIter2 first2, Sent2 last2, OutIter result, Comp comp, Tag)
{
   return (set_difference_scan)(first1, last1, first2, last2, result, comp, non_segmented_iterator_tag()).second;
}
   ...
   (InIter1 first1, Sent1 last1, InIter2 first2, Sent2 last2, OutIter result, Comp comp)
{
   ...
   return detail_algo::segmented_set_difference_dispatch
   ...
   (InIter1 first1, Sent1 last1, InIter2 first2, Sent2 last2, OutIter result)
{
   return boost::container::segmented_set_difference
      (first1, last1, first2, last2, result, detail_algo::set_difference_default_less());
}
