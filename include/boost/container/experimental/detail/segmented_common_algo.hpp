...
//
//////////////////////////////////////////////////////////////////////////////
#ifndef BOOST_CONTAINER_EXPERIMENTAL_DETAIL_SEGMENTED_COMMON_ALGO_HPP
#define BOOST_CONTAINER_EXPERIMENTAL_DETAIL_SEGMENTED_COMMON_ALGO_HPP
...
//Category tag dispatch value-initialises iterator categories and
//seg_is_ra_iterator tests convertibility between them, but std_fwd.hpp only
//forward-declares the std tag types, so they must be completed here.
#include <iterator>
...
#if defined(BOOST_CONTAINER_SEGMENTED_ENABLE_PRAGMA_UNROLL) && defined(BOOST_CONTAINER_SEGMENTED_DISABLE_PRAGMA_UNROLL)
#error "Can't define both BOOST_CONTAINER_SEGMENTED_ENABLE_PRAGMA_UNROLL and BOOST_CONTAINER_SEGMENTED_DISABLE_PRAGMA_UNROLL"
#elif defined(BOOST_CONTAINER_SEGMENTED_ENABLE_PRAGMA_UNROLL)
...
//! Default equality function object used by segmented algorithms when the
//! user does not provide an explicit binary predicate.  Centralised here to
//! avoid duplicating the same struct across every algorithm header.
struct segmented_default_equal_to
{
...
// to a constant true.
//
// Shared by segmented_mismatch and segmented_equal, which differ only in what
// they make of the final positions; the walk itself is the same.
//////////////////////////////////////////////////////////////////////////////
    ...
    const non_segmented_iterator_tag &, const std::random_access_iterator_tag &)
{
   ...
   //Derived from first1/last1 and not from src_n/iter2_n: comparing the two
   //lengths again would force the min above to be materialised as a value and
   //costs the cmov.
   stop = first1 == last1;
    ...
    segmented_iterator_tag, SrcCat)
{
   ...
   for(;;) {
      ...
      {
         const local_result_t r = (segmented_iter2_bounded)
...
#endif // BOOST_CONTAINER_EXPERIMENTAL_DETAIL_SEGMENTED_COMMON_ALGO_HPP
