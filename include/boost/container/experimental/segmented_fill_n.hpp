...
segduo<SegIt, Size> fill_n_scan(SegIt first, SegIt last, Size count, const T& value, segmented_iterator_tag, const Cat &)
{
   ...
   if(BOOST_CONTAINER_SEG_LIKELY(scur != slast)) {
      {
         const segduo<local_iterator, Size> r = fill_n_scan(lb, traits::end(scur), count, value, is_local_seg_t(), local_cat_t());
         count = r.second;
   ...
   const segduo<local_iterator, Size> r = (fill_n_scan)(lb, ll, count, value, is_local_seg_t(), local_cat_t());
