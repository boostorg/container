...
public:
   ...
   new_deque<recursive_deque> deque_;
   new_deque<recursive_deque>::iterator it_;
   new_deque<recursive_deque>::const_iterator cit_;
   new_deque<recursive_deque>::reverse_iterator rit_;
   new_deque<recursive_deque>::const_reverse_iterator crit_;
};
...
bool do_test()
{
   ...
   {
      new_deque<recursive_deque> recursive_deque_deque;
   }

   {
      //Now test move semantics
      new_deque<recursive_deque> original;
      new_deque<recursive_deque> move_ctor(boost::move(original));
      new_deque<recursive_deque> move_assign;
      move_assign = boost::move(move_ctor);
      move_assign.swap(original);
   }
   {
   typedef new_deque<IntType>  MyCntDeque;
   ...
   //Check Constructor Template Auto Deduction
   {
      ...
      auto test = new_deque(gold.begin(), gold.end());
      if(!test::CheckEqualContainers(gold, test)) return false;
   }
   {
      auto gold = MyStdDeque{ 1, 2, 3 };
      auto test = new_deque(gold.begin(), gold.end(), new_allocator<int>());
      if(!test::CheckEqualContainers(gold, test)) return false;
   }
#endif
...
struct boost_container_new_deque;

namespace boost { namespace container {   namespace test {

template<>
struct alloc_propagate_base<boost_container_new_deque>
{
   template <class T, class Allocator>
   struct apply
   {
      typedef boost::container::new_deque<T, Allocator> type;
   };
};
...
int main ()
{
   ...
   {
      new_deque<test::non_copymovable_int> d;
      d.emplace_back();
      d.emplace_front(1);
      d.resize(10);
      d.resize(1);
   ...
   //    Default init test
   ////////////////////////////////////
   if(!test::default_init_test< new_deque<int, test::default_init_allocator<int> > >()){
   ...
   if(!boost::container::test::test_emplace
      < new_deque<test::EmplaceInt>, Options>())
      return 1;
   ////////////////////////////////////
   //    Allocator propagation testing
   ////////////////////////////////////
   if(!boost::container::test::test_propagate_allocator<boost_container_new_deque>())
      return 1;
   ...
   ////////////////////////////////////
   {
      typedef boost::container::new_deque<int> cont_int;
   ...
   // default allocator
   {
      typedef boost::container::new_deque<int> cont;
   ...
   // std::allocator
   {
      typedef boost::container::new_deque<int, std::allocator<int> > cont;
      typedef cont::allocator_type allocator_type;
      typedef boost::container::allocator_traits<allocator_type>::pointer pointer;
      BOOST_CONTAINER_STATIC_ASSERT_MSG(!(boost::has_trivial_destructor_after_move<cont>::value !=
                               boost::has_trivial_destructor_after_move<allocator_type>::value &&
